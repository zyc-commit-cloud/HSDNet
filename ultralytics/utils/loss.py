...
#
# _MPDIOU_PRINTED = False
# def MPDIoU(box1, box2, xywh=False, img_w=None, img_h=None, eps=1e-7):
#     global _MPDIOU_PRINTED
#     if not _MPDIOU_PRINTED:
#         print("✅ 当前使用的是 MPDIoU loss")
#         _MPDIOU_PRINTED = True
...
#     # ✅ 自动计算实际坐标范围，不依赖固定的640
#     all_boxes = torch.cat([box1, box2], dim=0)
#     w = all_boxes[..., 2].max() - all_boxes[..., 0].min()
#     h = all_boxes[..., 3].max() - all_boxes[..., 1].min()
#     d_sq = w ** 2 + h ** 2 + eps

#     mpdiou = iou - (dist_sq / d_sq)
#     return torch.clamp(mpdiou, min=-1.0, max=1.0)
#___________

_INNERCIOU_PRINTED = False
def get_inner_iou(box1, box2, xywh=True, eps=1e-7, ratio=1.10):
    def xyxy2xywh(x):
        assert x.shape[-1] == 4
    ...
    b1_x1, b1_x2 = x1 - (w1 * ratio) / 2, x1 + (w1 * ratio) / 2
    b1_y1, b1_y2 = y1 - (h1 * ratio) / 2, y1 + (h1 * ratio) / 2
    b2_x1, b2_x2 = x2 - (w2 * ratio) / 2, x2 + (w2 * ratio) / 2
    b2_y1, b2_y2 = y2 - (h2 * ratio) / 2, y2 + (h2 * ratio) / 2
...
def bbox_inner_iou(box1, box2, xywh=True, CIoU=False, eps=1e-7, ratio=1.10):
    global _INNERCIOU_PRINTED
    if not _INNERCIOU_PRINTED:
        print("✅ 当前使用的是 Inner-CIoU loss")
        _INNERCIOU_PRINTED = True
...
class FocalLoss(nn.Module):
    ...
    def forward(pred, label, gamma=1.5, alpha=0.25):
        ...
        # p_t = torch.hyper_hazydet_200张_100epoch(-loss)
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        # MPDIOU去掉上两行，换以下
        # mpdiou = MPDIoU(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, img_w=640, img_h=640)
        # loss_iou = ((1.0 - mpdiou) * weight).sum() / target_scores_sum

        #innerciou
        iou = bbox_inner_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], 
                     xywh=False, CIoU=True, ratio=1.10)
